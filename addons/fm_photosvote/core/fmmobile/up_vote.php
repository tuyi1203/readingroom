...
*from_user 来自用户
tfrom_user 选手
http://sx.x.cqcbw.com/mywe/app/index.php?i=76&c=entry&vote=1&rid=1712&from_user=oOFaDjlQI8jXOy7q8m5ClcbFCvOg&do=up_vote&m=fm_photosvote&up_voteer=undefined&token=asd&tfrom_user=FMQKjD5vLqZ7J4TDlV1489057549&from_user=asdasd
*
投票记录表
ims_fm_photosvote_counter
ims_fm_photosvote_votelog
*/
$_GPC['vote'] = 1;
$_GPC['up_voteer'] = 'undefined';
$_GPC['message'] = 'nocode';
$_GPC['vfrom'] = 'photosvote';
	...
	if (!empty($paymore) && !empty($payordersn) && $rvote['votepay'] == 1 && $_W['account']['level'] == 4) {
		...
		if($_GPC['vote'] == '1' ) {
			...
			if (!$tpxz_status) {//每天总共投票的次数限制（全部人）
				...
				if (($rvote['votepay'] || $rvote['giftvote']) && $rvote['votenumpiao'] ) {
					if ($_GPC['votepay']) {
						...
						//$msg .= '<br />您也可以重新进入或刷新该页面使用付费投票或者给他送礼物';
						$fmdata = array(
							"success" => -3,
							"msg" => $msg,
						);
			...
			if ($_GPC['votepay'] == 1 && $_GPC['paystatus'] != 'success') {
				...
				$log = pdo_get('core_paylog', array('uniacid' => $u_uniacid, 'module' => $this->module['name'], 'tid' => $ordersn));
