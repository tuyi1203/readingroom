...
//评论开始
	...
	$comments = pdo_fetchall('SELECT * FROM '.tablename($this->table_bbsreply).' WHERE rid = :rid AND tfrom_user = :tfrom_user AND is_del = 0 ORDER BY `createtime` ASC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, array(':rid' => $rid,':tfrom_user' => $tfrom_user));
	$total = pdo_fetchcolumn('SELECT COUNT(1) FROM '.tablename($this->table_bbsreply).' WHERE rid = :rid AND tfrom_user = :tfrom_user AND is_del = 0 ', array(':rid' => $rid,':tfrom_user' => $tfrom_user));
	$total_pages = ceil($total/$psize);	
	$pager = paginationm($total, $pindex, $psize, '', array('before' => 0, 'after' => 0, 'ajaxcallback' => ''));

	$zan = pdo_fetch('SELECT zan FROM '.tablename($this->table_bbsreply).' WHERE rid = :rid AND tfrom_user = :tfrom_user AND from_user = :from_user AND zan = 1 AND is_del = 0  LIMIT 1' , array(':rid' => $rid,':tfrom_user' => $tfrom_user,':from_user' => $from_user));
//回复数
