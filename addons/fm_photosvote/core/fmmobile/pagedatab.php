...
$rid = $_GPC['rid'];
//查询活动是否开启了抽奖
$upload = pdo_fetch("SELECT isroll FROM " . tablename('fm_photosvote_wm_upload') . " WHERE rid=:rid limit 1",array(":rid"=>$rid));
		...
		foreach ($userlist as $key => $row) {
			...
			if( $upload['isroll'] == '1'){
				$userlist[$key]['avatar'] = $this->getphotos($row['avatar'], $row['avatar'], FM_STATIC_MOBILE . 'public/images/nofoundpic.gif');
			}else{
