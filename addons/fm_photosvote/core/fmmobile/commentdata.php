...
$comments = pdo_fetchall('SELECT * FROM ' . tablename($this -> table_bbsreply) . ' WHERE rid = :rid AND tfrom_user = :tfrom_user AND is_del = 0 ' . $where . ' ORDER BY `createtime` ASC LIMIT ' . $position . ',' . $item_per_page, array(':rid' => $rid, ':tfrom_user' => $tfrom_user));
