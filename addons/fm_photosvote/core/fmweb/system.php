...
$uploadTable = 'fm_photosvote_wm_upload';
...
if ($op == 'rshare') {
	...
	if (checksubmit('submit')) {
		...
		$insert_share = array(
			...
			'shareusertitle' => $_GPC['shareusertitle'],
			'shareusercontent' => $_GPC['shareusercontent'],
			'sharetoptitle' => $_GPC['sharetoptitle'],
			'sharetopcontent' => $_GPC['sharetopcontent'],
		);
		if (!empty($rshare['rid'])) {
	...
	$upload = pdo_fetch("SELECT * FROM ".tablename($uploadTable)." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
	if( $upload ){
		$upload['prize'] = iunserializer($upload['prize']);
		$rdisplay = array_merge($rdisplay,$upload);
		//print_r($_GPC['prize']);exit;
	}
	if (!empty($rdisplay['regtitlearr'])) {
		$regtitlearr = iunserializer($rdisplay['regtitlearr']);
	}
	...
	$rdisplay['isappendpic'] = !isset($rdisplay['isappendpic']) ? "0" : $rdisplay['isappendpic'];
	$rdisplay['isroll'] = !isset($rdisplay['isroll']) ? "0" : $rdisplay['isroll'];	
	$rdisplay['piczoom'] = !isset($rdisplay['piczoom']) ? "300" : $rdisplay['piczoom'];
	$rdisplay['islottery'] = !isset($rdisplay['islottery']) ? "0" : $rdisplay['islottery'];
	$rdisplay['lotterynum'] = !isset($rdisplay['lotterynum']) ? "3" : $rdisplay['lotterynum'];
	$rdisplay['filed1name'] = !isset($rdisplay['filed1name']) ? "姓名" : $rdisplay['filed1name'];
	$rdisplay['filed2name'] = !isset($rdisplay['filed2name']) ? "电话" : $rdisplay['filed2name'];
	$rdisplay['filed3name'] = !isset($rdisplay['filed3name']) ? "拍摄地" : $rdisplay['filed3name'];
	$rdisplay['filed4name'] = !isset($rdisplay['filed4name']) ? "拍摄时间" : $rdisplay['filed4name'];
	$rdisplay['filed5name'] = !isset($rdisplay['filed5name']) ? "宣言" : $rdisplay['filed5name'];
	$rdisplay['pagecount'] = !isset($rdisplay['pagecount']) ? "10" : $rdisplay['pagecount'];
	$rdisplay['lotteryrepeat'] = !isset($rdisplay['lotteryrepeat']) ? "0" : $rdisplay['lotteryrepeat'];
	$rdisplay['lotterycontinue'] = !isset($rdisplay['lotterycontinue']) ? "0" : $rdisplay['lotterycontinue'];
	$rdisplay['lotterystart'] = !isset($rdisplay['lotterystart']) ? "5" : $rdisplay['lotterystart'];
	...
	if (checksubmit('submit')) {
		...
		$upload_data = array(
			'rid' => $rid,
			'isroll' => $_GPC['isroll'] == 'on' ? 1 : 0,
			'pagecount' => $_GPC['pagecount'] == '' ? 10 : $_GPC['pagecount'],
			'isappendpic' => $_GPC['isappendpic'] == 'on' ? 1 : 0,
			'piczoom' => $_GPC['piczoom'] == '' ? 300 : $_GPC['piczoom'],
			'islottery' => $_GPC['islottery'] == 'on' ? 1 : 0,
			'lotteryrepeat' => $_GPC['lotteryrepeat'] == 'on' ? 1 : 0,
			'lotterycontinue' => $_GPC['lotterycontinue'] == 'on' ? 1 : 0,
			'lotterystart' => $_GPC['lotterystart'] == '' ? 4 : $_GPC['lotterystart'],
			'lotteryrull' => htmlspecialchars_decode($_GPC['lotteryrull']),
			'lotterynum' => $_GPC['lotterynum'] == '' ? 3 : $_GPC['lotterynum'],
			'prize' => iserializer($_GPC['prize']),
			'filed1name' => $_GPC['filed1name'],
			'filed1just' => $_GPC['filed1just'],
			'filed2name' => $_GPC['filed2name'],
			'filed2just' => $_GPC['filed2just'],
			'filed3name' => $_GPC['filed3name'],
			'filed3just' => $_GPC['filed3just'],
			'filed4name' => $_GPC['filed4name'],
			'filed4just' => $_GPC['filed4just'],
			'filed5name' => $_GPC['filed5name'],
			'filed5just' => $_GPC['filed5just'],
		);

		if (!empty($rdisplay['rid'])) {
			pdo_update($this->table_reply_display, $insert_display, array('rid' => $rid));
			if( !$upload ){
				pdo_insert($uploadTable, $upload_data);
			}
			else{
				pdo_update($uploadTable, $upload_data, array('rid' => $rid));
			}
		} else {
