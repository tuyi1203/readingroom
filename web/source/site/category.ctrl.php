...
if ($do == 'display') {
	...
	$children = array();
	$subchildren = array();
	$category = $category_copy = pdo_fetchall("SELECT * FROM ".tablename('site_category')." WHERE uniacid = '{$_W['uniacid']}' ORDER BY parentid, displayorder DESC, id");
	// 2018.7.31 tuyi 修改为3级
	foreach ($category as $index => $row) 
	{
		if (!empty($row['parentid']))
		{
			if (!grandparentid_exists($row['parentid'],$category_copy))
			{
				$children[$row['parentid']][] = $row;
			}
			else
			{
				$subchildren[$row['parentid']][] = $row;
			}
