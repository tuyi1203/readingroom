...
if ($do == 'display') {
  ...
  if (!empty($_GPC['daterange'])) {
    ...
    $date_to = trim($date[1]);
    $condition .= " and ( a.date between :date_from and :date_to )";
  ...
  $sql = "SELECT a.id,a.date,a.book_type,a.time_section,a.create_time,b.name,c.time_from,c.time_to,c.comment FROM " . tablename('jm_reading_room_booking_log') . "a INNER JOIN " . tablename('jm_users') . " b on a.uid = b.id INNER JOIN " . tablename('jm_reading_time_section') . "c on a.time_section=c.id $condition ";

  $orderby = " ORDER BY a.date DESC,c.sort ";

  $limit = " LIMIT " . ($pindex - 1) * $psize . ',' . $psize;

  $list = pdo_fetchall($sql . $orderby . $limit, $params);

  if (!empty($list)) {
    foreach ($list as $index => $record) {
      $list[$index]['book_type_name'] = $bookTypeList[$record['book_type']];
    }
  }

  $count_sql = "SELECT count(a.id) FROM " . tablename('jm_reading_room_booking_log') . "a INNER JOIN " . tablename('jm_users') . " b on a.uid = b.id INNER JOIN " . tablename('jm_reading_time_section') . "c on a.time_section=c.id $condition ";
  ...
  load()->func('file');
  $id = intval($_GPC['id']);
  $row = pdo_fetch("SELECT count(*) FROM " . tablename('jm_reading_room_booking_log') . " WHERE id = :id", array(':id' => $id));
  if (empty($row)) {
  ...
  pdo_delete('jm_reading_room_booking_log', array('id' => $id));
