		...
		$.fn.poptrox = function(options) {
			...
			// Settings
				var settings = $.extend({
					...
					overlayOpacity:					0.9,						// Overlay opacity
					...
					popupCloserBackgroundColor:		'none',					// (Default Style) Popup closer background color (when usePopupStyling = true)
					popupCloserWidth:               '40px',
					popupCloserHeight:              '40px',
					popupCloserBottom:              '-60px',
					popupCloserLeft:                '50%',
					popupCloserMarginLeft:          '-20px',
					popupCloserTextColor:			'#FFFFFF',					// (Default Style) Popup closer text color (when usePopupStyling = true)
					popupCloserTextSize:			'30px',						// (Default Style) Popup closer text size
					popupPadding:					10,							// (Default Style) Popup padding (when usePopupStyling = true)
					popupCaptionHeight:				10,							// (Default Style) Popup height of caption area
					popupCaptionTextSize:			null,						// (Default Style) Popup caption text size
					popupCaptionTextColor:          '#000000',
					popupCaptionMarginBottom:       '0',
					popupCaptionPaddingTop:         '10px',
			...
			// Variables
				...
				function updateWH() {
					...
					$popup
						.css('max-width', settings.popupWidth)
					...
					if (settings.usePopupDefaultStyling) {
						...
						if ($caption.length > 0) {
							...
							$caption
								...
								.css('margin-bottom',settings.popupCaptionMarginBottom)
								.css('color',settings.popupCaptionTextColor)
								.css('padding-top',settings.popupCaptionPaddingTop)
						...
						if ($closer.length > 0)
							$closer
								...
								.css('display', 'block')
								.css('width', settings.popupCloserWidth)
								.css('height', settings.popupCloserHeight)
								.css('line-height', settings.popupCloserHeight)
								.css('text-align', 'center')
								.css('position', 'absolute')
								.css('text-decoration', 'none')
								.css('outline', '0')
								.css('bottom', settings.popupCloserBottom)
								.css('left', settings.popupCloserLeft)
								.css('margin-left',settings.popupCloserMarginLeft);
				...
				$this.find(settings.selector).each(function(index) {
					...
					// Create object (based on type)
						...
						switch (x.type) {
							...
							case 'video':
								x.object = $('<video controls="controls" autoplay="autoplay"></video>');
								x.src = x.src;
