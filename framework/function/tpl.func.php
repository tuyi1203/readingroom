...
function tpl_form_field_category_3level($name, $parents, $children, $parentid, $childid , $subchildren, $subchildid){
	$html = '
		<script type="text/javascript">
			window._' . $name . ' = ' . json_encode($children) . ';
			window._sub' . $name . ' = ' . json_encode($subchildren) . ';
		</script>';
			if (!defined('TPL_INIT_CATEGORY')) {
				$html .= '
		<script type="text/javascript">
			...
			function renderCategory_child(obj, name){
				var index = obj.options[obj.selectedIndex].value;
				require([\'jquery\', \'util\'], function($, u){
					$selectChild = $(\'#\'+name+\'_subchild\');
					var html = \'<option value="0">请选择三级分类</option>\';
					if (!window[\'_sub\'+name] || !window[\'_sub\'+name][index]) {
						$selectChild.html(html);
						return false;
					}
					for(var i=0; i< window[\'_sub\'+name][index].length; i++){
						html += \'<option value="\'+window[\'_sub\'+name][index][i][\'id\']+\'">\'+window[\'_sub\'+name][index][i][\'name\']+\'</option>\';
					}
					$selectChild.html(html);
				});
			}
			...
			<div class="col-xs-8 col-sm-4 col-md-4 col-lg-4">
				<select class="form-control tpl-category-child" id="' . $name . '_child" name="' . $name . '[childid]" onchange="renderCategory_child(this,\'' . $name . '\')">
					<option value="0">请选择二级分类</option>';
			if (!empty($parentid) && !empty($children[$parentid])) {
				foreach ($children[$parentid] as $row) {
					$html .= '
			...
			$html .= '
			<div class="col-xs-8 col-sm-4 col-md-4 col-lg-4">
				<select class="form-control tpl-category-child" id="' . $name . '_subchild" name="' . $name . '[subchildid]">
					<option value="0">请选择三级分类</option>';
			if (!empty($childid) && !empty($subchildren[$childid])) 
			{
				foreach ($subchildren[$childid] as $row)
				{
					$html .= '
					<option value="' . $row['id'] . '"' . (($row['id'] == $subchildid) ? 'selected="selected"' : '') . '>' . $row['name'] . '</option>';
				}
			}
			$html .= '
				</select>
